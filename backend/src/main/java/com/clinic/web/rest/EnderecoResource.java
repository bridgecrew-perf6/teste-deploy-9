...
/**
 * REST controller for managing {@link com.clinic.domain.Endereco}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class EnderecoResource {
    ...
    public List<Endereco> getAllEnderecos(@RequestParam(required = false) String filter) {
        if ("clinica-is-null".equals(filter)) {
            log.debug("REST request to get all Enderecos where clinica is null");
            return StreamSupport
                .stream(enderecoRepository.findAll().spliterator(), false)
                .filter(endereco -> endereco.getClinica() == null)
        ...
        if ("paciente-is-null".equals(filter)) {
            ...
            return StreamSupport
                .stream(enderecoRepository.findAll().spliterator(), false)
                .filter(endereco -> endereco.getPaciente() == null)
                .collect(Collectors.toList());
        }
        if ("profissional-is-null".equals(filter)) {
            log.debug("REST request to get all Enderecos where profissional is null");
            return StreamSupport
                .stream(enderecoRepository.findAll().spliterator(), false)
                .filter(endereco -> endereco.getProfissional() == null)
                .collect(Collectors.toList());
        }
        if ("funcionario-is-null".equals(filter)) {
            log.debug("REST request to get all Enderecos where funcionario is null");
            return StreamSupport
                .stream(enderecoRepository.findAll().spliterator(), false)
                .filter(endereco -> endereco.getFuncionario() == null)
                .collect(Collectors.toList());
        }
